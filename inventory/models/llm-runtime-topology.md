...
|---|---|---:|---|---|
| `qwen3-coder` | `llama.cpp` server, Vulkan container family | normally `8082` from prior validation context | Strix coder model endpoint | Unknown from snapshot |
...
- Current coding execution host.
- Hosts the primary local coder endpoint and backup local model endpoint for
  the self-hosted coding workflow.

Stable LLM services:

| Service/container | Runtime/image family | Endpoint/port | Role/purpose | Source/config path hints |
|---|---|---:|---|---|
| `qwen3-coder-30b` | `llama.cpp` `server-cuda` container family | `8083` | Primary AMD local coder endpoint | Unknown from snapshot |
| `gemma4-7900xt` | `llama.cpp` `server-rocm` container family | `8084` | AMD RX 7900 XT backup/small-model endpoint | Unknown from snapshot |
