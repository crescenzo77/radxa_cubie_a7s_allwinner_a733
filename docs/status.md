...
`W=1` on a Linux development build host. The Radxa Cubie A7S board-compatible
...
object now compile-tests cleanly with `W=1` on a Linux development build host.
...
schema. The integrated branch also compile-tests both A733 driver objects with
`W=1` and an arm64 cross compiler on `strix`, and the four binding checks pass
in the validated schema flow. The integrated branch also builds a full arm64
defconfig `Image` on `strix`.
